...
export default function CorporateActionsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const actions = await corporateActionsAPI.getActiveActions(myCorp.id).catch(() =>
          corporateActionsAPI.getAllActions(myCorp.id).catch(() => [])
        );
  ...
  const marketCap = useMemo(() => {
    ...
    const totalHeldByPlayers =
      corporation.shareholders?.reduce((sum: number, sh: any) => sum + (sh.shares || 0), 0) || 0;
  ...
  const handleActivateSupplyRush = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Failed to activate Supply Rush.');
    } finally {
      setActivating(null);
    }
  };
  ...
  const handleActivateMarketingCampaign = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Failed to activate Marketing Campaign.');
    } finally {
      setActivating(null);
    }
  };
    ...
    return (
      <AppNavigation>
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center">
          <div className="text-lg text-gray-600 dark:text-gray-200">Loading corporate actions...</div>
        </div>
      </AppNavigation>
    );
  }
