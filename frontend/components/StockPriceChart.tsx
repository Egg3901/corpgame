...
import { sharesAPI, SharePriceHistoryResponse } from '@/lib/api';
import {
  AreaChart,
  Area,
  XAxis,
...
export default function StockPriceChart({ corporationId, currentPrice }: StockPriceChartProps) {
  const [timeFrame, setTimeFrame] = useState<TimeFrame>('96h');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [historyData, setHistoryData] = useState<SharePriceHistoryResponse[]>([]);
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      setError(null);
      try {
        const data = await sharesAPI.getPriceHistory(
          corporationId,
          selectedTimeFrame.hours,
          1000 // Get enough data points
        );
        setHistoryData(data);
      } catch (err: any) {
    ...
    fetchHistory();
  }, [corporationId, selectedTimeFrame.hours]);
  ...
  const chartData = useMemo((): ChartDataPoint[] => {
    if (historyData.length === 0) {
      ...
      const now = Date.now();
      return [
        { time: 'Start', price: currentPrice, timestamp: now - selectedTimeFrame.hours * 3600000, capital: 0 },
        { time: 'Now', price: currentPrice, timestamp: now, capital: 0 },
      ];
    }
    ...
    return sorted.map((point) => {
      ...
      return {
        time: timeLabel,
        price: typeof point.share_price === 'string' ? parseFloat(point.share_price) : point.share_price,
        timestamp: date.getTime(),
        capital: typeof point.capital === 'string' ? parseFloat(point.capital) : point.capital,
      };
    });
  ...
  const gradientId = `priceGradient-${corporationId}`;
      ...
      <div className="relative p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Stock Price</h2>
