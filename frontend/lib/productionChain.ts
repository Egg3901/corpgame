 ...
 *
 * FID-20251225-008
 * Updated: FID-20251228-001 - Now supports dynamic configuration from unified sector config
 */

import { Node, Edge, MarkerType } from 'reactflow';
import { UnifiedSectorConfig } from '@/lib/api';
...
export const SECTORS = [
  ...
  'Manufacturing',      // LEGACY - Now migrated to Light Industry, kept for backwards compatibility
  'Energy',
  'Retail',
  'Real Estate',
  'Transportation',
...
const SECTOR_RESOURCES: Record<Sector, Resource | null> = {
  ...
  'Manufacturing': null,              // LEGACY - Same as Light Industry, consumes Steel PRODUCT
...
// ============================================================================
// CONFIG-AWARE FUNCTIONS (Use unified sector config when available)
// FID-20251228-001
// ============================================================================

/**
 * Config-aware helper: Get sectors producing a specific product
 */
function getSectorsProducingProductFromConfig(config: UnifiedSectorConfig, product: string): string[] {
  const result: string[] = [];
  for (const [sectorName, sector] of Object.entries(config.sectors)) {
    if (sector.producedProduct === product) {
...
/**
 * Config-aware helper: Get sectors demanding a specific product
 */
function getSectorsDemandingProductFromConfig(config: UnifiedSectorConfig, product: string): Array<{ sector: string; unitTypes: UnitType[] }> {
  ...
  for (const [sectorName, sector] of Object.entries(config.sectors)) {
    ...
    for (const unitType of ['production', 'retail', 'service', 'extraction'] as UnitType[]) {
      ...
      if (unit.isEnabled && unit.inputs.some(i => i.type === 'product' && i.name === product)) {
...
/**
 * Config-aware helper: Get sectors extracting a specific resource
 */
function getSectorsExtractingResourceFromConfig(config: UnifiedSectorConfig, resource: string): string[] {
  const result: string[] = [];
  for (const [sectorName, sector] of Object.entries(config.sectors)) {
    const extractionOutputs = sector.units.extraction.outputs;
    if (extractionOutputs.some(o => o.type === 'resource' && o.name === resource)) {
      result.push(sectorName);
    }
  }
  return result;
...
/**
 * Config-aware helper: Get sectors consuming a specific resource
 */
function getSectorsConsumingResourceFromConfig(config: UnifiedSectorConfig, resource: string): Array<{ sector: string; unitTypes: UnitType[] }> {
  const result: Array<{ sector: string; unitTypes: UnitType[] }> = [];
  ...
  for (const [sectorName, sector] of Object.entries(config.sectors)) {
    ...
    for (const unitType of ['production', 'retail', 'service', 'extraction'] as UnitType[]) {
      const unit = sector.units[unitType];
      if (unit.isEnabled && unit.inputs.some(i => i.type === 'resource' && i.name === resource)) {
        unitTypes.push(unitType);
      }
    }
...
/**
 * Config-aware helper: Get input resources for a sector
 */
function getInputResourcesForSectorFromConfig(config: UnifiedSectorConfig, sectorName: string): string[] {
  const sector = config.sectors[sectorName];
  if (!sector) return [];

  const resources: string[] = [];
  for (const input of sector.units.production.inputs) {
    if (input.type === 'resource') {
      resources.push(input.name);
    }
  }
  return resources;
}
...
/**
 * Config-aware helper: Get output product for a sector
 */
function getOutputProductForSectorFromConfig(config: UnifiedSectorConfig, sectorName: string): string | null {
  const sector = config.sectors[sectorName];
  return sector?.producedProduct || null;
}

/**
 * Config-aware version: Get production chain data for a product
 */
function getProductChainFromConfig(config: UnifiedSectorConfig, product: string): ProductionChainData {
  const nodes: Node<ChainNodeData>[] = [];
  const edges: Edge[] = [];

  const producingSectors = getSectorsProducingProductFromConfig(config, product);
  const demandingSectors = getSectorsDemandingProductFromConfig(config, product);

  // Collect all input resources for producing sectors
  const inputResources = new Set<string>();
  for (const sector of producingSectors) {
    const resources = getInputResourcesForSectorFromConfig(config, sector);
    resources.forEach(r => inputResources.add(r));
  }
  ...
  producingSectors.forEach((sector, idx) => {
    ...
    // Create edges from resources to this producer
    const sectorResources = getInputResourcesForSectorFromConfig(config, sector);
    sectorResources.forEach(resource => {
      if (resourceNodes[resource]) {
        edges.push({
          id: `edge-${resourceNodes[resource]}-${id}`,
  ...
  demandingSectors.slice(0, 8).forEach((item, idx) => {
    ...
    nodes.push({
      ...
      data: {
        label: item.sector,
        nodeType: 'sector',
        unitTypes: item.unitTypes as UnitType[],
        role: 'consumer',
      },
    });
...
/**
 * Config-aware version: Get production chain data for a resource
 */
function getResourceChainFromConfig(config: UnifiedSectorConfig, resource: string): ProductionChainData {
  const nodes: Node<ChainNodeData>[] = [];
  const edges: Edge[] = [];

  const extractingSectors = getSectorsExtractingResourceFromConfig(config, resource);
  const consumingSectors = getSectorsConsumingResourceFromConfig(config, resource);

  // Collect output products from consuming sectors
  const outputProducts = new Set<string>();
  for (const { sector } of consumingSectors) {
    const product = getOutputProductForSectorFromConfig(config, sector);
    if (product) {
      outputProducts.add(product);
    }
  }
  ...
  productArray.forEach((product, idx) => {
    ...
    // Find which consuming sectors produce this product
    for (const { sector } of consumingSectors) {
      if (getOutputProductForSectorFromConfig(config, sector) === product && consumerNodes[sector]) {
        edges.push({
          id: `edge-${consumerNodes[sector]}-${id}`,
          source: consumerNodes[sector],
          target: id,
...
/**
 * Get production chain data using unified sector config
 * Falls back to hardcoded constants if config is not provided
 *
 * @param type - 'product' or 'resource'
 * @param name - The name of the product or resource
 * @param config - Optional unified sector config from useSectorConfig hook
 */
export function getProductionChainDataWithConfig(
  type: 'product' | 'resource',
  name: string,
  config?: UnifiedSectorConfig | null
): ProductionChainData {
  // If config is provided, use config-aware functions
  if (config) {
    if (type === 'product') {
      return getProductChainFromConfig(config, name);
    } else {
      return getResourceChainFromConfig(config, name);
    }
  }

  // Fall back to original hardcoded version
  return getProductionChainData(type, name);
}

// Export config-aware lookup functions for use in components
export function getSectorsProducingProductWithConfig(config: UnifiedSectorConfig | null, product: string): string[] {
  if (config) {
    return getSectorsProducingProductFromConfig(config, product);
  }
  return getSectorsProducingProduct(product as Product);
}

export function getSectorsDemandingProductWithConfig(config: UnifiedSectorConfig | null, product: string): Array<{ sector: string; unitTypes: UnitType[] }> {
  if (config) {
    return getSectorsDemandingProductFromConfig(config, product);
  }
  return getSectorsDemandingProduct(product as Product);
}

export function getSectorsExtractingResourceWithConfig(config: UnifiedSectorConfig | null, resource: string): string[] {
  if (config) {
    return getSectorsExtractingResourceFromConfig(config, resource);
  }
  return getSectorsExtractingResource(resource as Resource);
}

export function getSectorsConsumingResourceWithConfig(config: UnifiedSectorConfig | null, resource: string): Array<{ sector: string; unitTypes: UnitType[] }> {
  if (config) {
    return getSectorsConsumingResourceFromConfig(config, resource);
  }
  return getSectorsConsumingResource(resource as Resource);
}
