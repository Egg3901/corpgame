...
const REGIONS_SOURCE = {
  ...
  'South': ['DE', 'FL', 'GA', 'MD', 'NC', 'SC', 'VA', 'WV', 'KY', 'TN', 'AL', 'MS', 'AR', 'LA', 'OK', 'TX', 'DC'],
...
module.exports = async function seedStates(db) {
  const collection = db.collection('state_metadata');

  // Prepare documents
  const documents = US_STATES.map(state => {
    const region = REGION_MAPPING[state.value] || 'Other';
    ...
    return {
      ...
      image_url: `/images/states/${state.value.toLowerCase()}.jpg` 
    };
  });

  // Clear existing (though master-reset should have handled this, safer to double check or just insert)
  // We assume master-reset cleared it, but this function can be standalone.
  // Using deleteMany again is safe.
  await collection.deleteMany({});
  
  const result = await collection.insertMany(documents);
  console.log(`Seeded ${result.insertedCount} states`);
};
