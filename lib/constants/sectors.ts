...
export const ENERGY_INPUTS: Partial<Record<Resource, number>> = {
...
export const PRODUCTION_ONLY_SECTORS: Sector[] = ['Heavy Industry', 'Mining', 'Light Industry'];
...
export function calculateCommodityPrice(
  ...
  const currentPrice = Math.max(10, rawPrice); // Use minimum floor
...
export function getResourceInfo(resource: Resource): {
  ...
  // Also check special inputs (Heavy Industry, Energy)
  if (HEAVY_INDUSTRY_INPUTS[resource]) {
    if (!demandingSectors.includes('Heavy Industry')) demandingSectors.push('Heavy Industry');
  }
  if (ENERGY_INPUTS[resource]) {
    if (!demandingSectors.includes('Energy')) demandingSectors.push('Energy');
  }
  
  return {
    name: resource,
    basePrice: RESOURCE_BASE_PRICES[resource] || 100,
    extractingSectors,
    demandingSectors,
  };
}
...
export interface DynamicUnitEconomics {
  ...
  resourceConsumedAmounts: Partial<Record<Resource, number>>;  // Amount of each resource consumed
  ...
  productConsumedAmounts: Partial<Record<Product, number>>;  // Amount of each product consumed
...
export function getDynamicUnitEconomics(
  ...
  if (unitType === 'extraction') {
    ...
    // Use first extractable resource for revenue calc (most are single-resource)
    const resource = extractableResources[0];
    const commodityPrice = getCommodityUnitPrice(resource);
    
    const extractionRate = EXTRACTION_OUTPUT_RATE;
    const revenue = extractionRate * commodityPrice;

    // Calculate electricity cost
    const electricityPrice = getProductUnitPrice('Electricity');
    const electricityCost = EXTRACTION_ELECTRICITY_CONSUMPTION * electricityPrice;

    // Calculate other input costs
    let otherInputCost = 0;
    const productsConsumed: Product[] = ['Electricity'];
    const productConsumedAmounts: Partial<Record<Product, number>> = {
      'Electricity': EXTRACTION_ELECTRICITY_CONSUMPTION
    };

    if (sectorTyped === 'Mining') {
      const manufacturedGoodsPrice = getProductUnitPrice('Manufactured Goods');
      const cost = MINING_MANUFACTURED_GOODS_CONSUMPTION * manufacturedGoodsPrice;
      otherInputCost += cost;
      productsConsumed.push('Manufactured Goods');
      productConsumedAmounts['Manufactured Goods'] = MINING_MANUFACTURED_GOODS_CONSUMPTION;
    }

    const totalProductCost = electricityCost + otherInputCost;
    const totalCost = laborCost + totalProductCost;
    const hourlyProfit = revenue - totalCost;
    ...
    const result: DynamicUnitEconomics = {
      ...
      resourceConsumedAmount: 0, // Extraction produces resources, doesn't consume them
      resourcesConsumed: [],
      resourceConsumedAmounts: {} as Record<Resource, number>,
      productRevenue: 0,
      productProduced: null,
  ...
  // Production units: cost = labor + resources, revenue = products
  if (unitType === 'production') {
    // If sector doesn't produce products, return default (shouldn't happen for valid production sectors)
    if (!producedProduct) {
      if (canUseCache) {
        _dynamicEconomicsCache.set(cacheKey, defaultResult);
      }
    ...
    const productPrice = getProductUnitPrice(producedProduct);
    const revenue = PRODUCTION_OUTPUT_RATE * productPrice;
    ...
    // Handle single resource requirement
    if (requiredResource) {
      const commodityPrice = getCommodityUnitPrice(requiredResource);
      const consumptionRate = PRODUCTION_RESOURCE_CONSUMPTION;
      resourceCost += consumptionRate * commodityPrice;
      resourceConsumedAmount = consumptionRate;
      resourcesConsumed.push(requiredResource);
      resourceConsumedAmounts[requiredResource] = consumptionRate;
    }
    
    // Handle special multi-resource sectors
    if (sectorTyped === 'Heavy Industry') {
      for (const [res, amount] of Object.entries(HEAVY_INDUSTRY_INPUTS)) {
        if (amount) {
          const resType = res as Resource;
          const price = getCommodityUnitPrice(resType);
          resourceCost += amount * price;
          if (!resourcesConsumed.includes(resType)) {
            resourcesConsumed.push(resType);
          }
          resourceConsumedAmounts[resType] = (resourceConsumedAmounts[resType] || 0) + amount;
        }
      }
    } else if (sectorTyped === 'Energy') {
      for (const [res, amount] of Object.entries(ENERGY_INPUTS)) {
        if (amount) {
          const resType = res as Resource;
          const price = getCommodityUnitPrice(resType);
          resourceCost += amount * price;
    ...
    } else if (sectorTyped === 'Construction') {
      // Construction consumes Lumber resource directly
      const lumberPrice = getCommodityUnitPrice('Lumber');
      const lumberCost = CONSTRUCTION_LUMBER_CONSUMPTION * lumberPrice;
      resourceCost += lumberCost;
      resourcesConsumed.push('Lumber');
      resourceConsumedAmounts['Lumber'] = CONSTRUCTION_LUMBER_CONSUMPTION;
    }
    
    // Handle product inputs (e.g. Light Industry needs Steel)
    let productInputCost = 0;
    const productsConsumed: Product[] = ['Electricity'];
    const productConsumedAmounts: Partial<Record<Product, number>> = {};

    // Electricity cost
    const electricityPrice = getProductUnitPrice('Electricity');
    const elecConsumption = sectorTyped === 'Heavy Industry' 
      ? HEAVY_INDUSTRY_ELECTRICITY_CONSUMPTION 
      : PRODUCTION_ELECTRICITY_CONSUMPTION;
      
    productInputCost += elecConsumption * electricityPrice;
    productConsumedAmounts['Electricity'] = elecConsumption;

    const demandedProducts = SECTOR_PRODUCT_DEMANDS[sectorTyped];
    if (demandedProducts) {
      for (const product of demandedProducts) {
        const inputPrice = getProductUnitPrice(product);
        let consumption = PRODUCTION_PRODUCT_CONSUMPTION;
        
        // Special cases
        if (sectorTyped === 'Construction') {
          if (product === 'Steel') consumption = CONSTRUCTION_STEEL_CONSUMPTION;
        } else if (sectorTyped === 'Agriculture' && product === 'Manufactured Goods') {
          consumption = AGRICULTURE_MANUFACTURED_GOODS_CONSUMPTION;
        } else if (sectorTyped === 'Pharmaceuticals' && product === 'Technology Products') {
          consumption = PHARMACEUTICALS_TECHNOLOGY_CONSUMPTION;
        } else if (sectorTyped === 'Defense' && product === 'Technology Products') {
          consumption = DEFENSE_TECHNOLOGY_CONSUMPTION;
        }
        
        productInputCost += consumption * inputPrice;
        productsConsumed.push(product);
        productConsumedAmounts[product] = consumption;
      }
    }
    
    // Additional logistics costs for specific sectors
    if (sectorTyped === 'Energy') {
      const logisticsPrice = getProductUnitPrice('Logistics Capacity');
      productInputCost += ENERGY_LOGISTICS_CONSUMPTION * logisticsPrice;
      productsConsumed.push('Logistics Capacity');
      productConsumedAmounts['Logistics Capacity'] = ENERGY_LOGISTICS_CONSUMPTION;
    } else if (sectorTyped === 'Light Industry' || sectorTyped === 'Heavy Industry') {
      const logisticsPrice = getProductUnitPrice('Logistics Capacity');
      productInputCost += MANUFACTURING_LOGISTICS_CONSUMPTION * logisticsPrice;
      productsConsumed.push('Logistics Capacity');
      productConsumedAmounts['Logistics Capacity'] = MANUFACTURING_LOGISTICS_CONSUMPTION;
    }

    const totalCost = laborCost + resourceCost + productInputCost;
    const hourlyProfit = revenue - totalCost;
...
export function calculateMarketEntryEconomics(
  ...
  hourlyCost: number;
  hourlyProfit: number;
  totalRevenue: number; // Alias for hourlyRevenue
  totalCost: number;    // Alias for hourlyCost
} {
  let hourlyRevenue = 0;
  let hourlyCost = 0;
  ...
  // Retail
  if (retailCount > 0) {
    const eco = getDynamicUnitEconomics('retail', sector, marketPrices);
    hourlyRevenue += eco.hourlyRevenue * retailCount;
    hourlyCost += eco.hourlyCost * retailCount;
  }

  // Production
  if (productionCount > 0) {
    const eco = getDynamicUnitEconomics('production', sector, marketPrices);
    hourlyRevenue += eco.hourlyRevenue * productionCount;
    hourlyCost += eco.hourlyCost * productionCount;
  }

  // Service
  if (serviceCount > 0) {
    const eco = getDynamicUnitEconomics('service', sector, marketPrices);
    hourlyRevenue += eco.hourlyRevenue * serviceCount;
    hourlyCost += eco.hourlyCost * serviceCount;
  }

  // Extraction
  if (extractionCount > 0) {
    const eco = getDynamicUnitEconomics('extraction', sector, marketPrices);
    hourlyRevenue += eco.hourlyRevenue * extractionCount;
    hourlyCost += eco.hourlyCost * extractionCount;
  }
...
export const STOCK_VALUATION = {
  ...
  RECENCY_DECAY: 0.95,
  MIN_SHARE_PRICE: 0.10,
  HOURLY_VARIATION_PERCENT: 0.05,
};

export function getUnitAssetValue(unitType: string): number {
  return BUILD_UNIT_COST;
}
...
export function getMarketEntryAssetValue(
  ...
  const retailValue = retailCount * BUILD_UNIT_COST;
  const productionValue = productionCount * BUILD_UNIT_COST;
  const serviceValue = serviceCount * BUILD_UNIT_COST;
  const extractionValue = extractionCount * BUILD_UNIT_COST;
  
  const totalValue = retailValue + productionValue + serviceValue + extractionValue;
