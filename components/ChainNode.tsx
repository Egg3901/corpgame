...
function ChainNode({ data }: NodeProps<ChainNodeData>) {
  ...
  return (
    <Card
      isPressable={!!data.href}
      onPress={handleClick}
      className={`
        min-w-[100px] max-w-[160px] border-2 overflow-visible
        ${getBackgroundColor()}
        ${isCentral ? 'ring-2 ring-offset-2 ring-blue-500 dark:ring-offset-gray-900' : ''}
      `}
      shadow="sm"
    >
      <CardBody className="px-3 py-2 overflow-visible relative">
        {/* Incoming handle */}
        <Handle
          type="target"
            ...
            <div className="flex flex-wrap gap-1 justify-center mt-1">
              {data.unitTypes.map(unitType => (
                <Chip
                  key={unitType}
                  size="sm"
                  className="h-5 text-[10px] text-white px-0"
