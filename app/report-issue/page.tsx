...
import { Button, Textarea, Input, Select, SelectItem } from "@heroui/react";
...
export default function ReportIssuePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, `Failed to submit ${mode === 'issue' ? 'issue report' : 'suggestion'}`));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6">
          <div className="inline-flex rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 p-1 gap-1">
            <Button
              type="button"
              onPress={() => handleModeChange('issue')}
              ...
              variant={mode === 'issue' ? 'solid' : 'light'}
            >
            ...
            <Button
              type="button"
              onPress={() => handleModeChange('suggestion')}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold transition-all ${
                mode === 'suggestion'
                  ? 'bg-white dark:bg-gray-700 text-corporate-blue shadow-sm'
                  : 'bg-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
              }`}
              variant={mode === 'suggestion' ? 'solid' : 'light'}
            >
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-lg">
          <div className="space-y-6">
            ...
            <Input
              label={mode === 'issue' ? 'Issue Title' : 'Suggestion Title'}
            ...
            <Select
              label={mode === 'issue' ? 'Priority' : 'Importance'}
              labelPlacement="outside"
            ...
            >
              <SelectItem key="low">
                {mode === 'issue' ? 'Low' : 'Nice to have'}
              </SelectItem>
              <SelectItem key="medium">
                {mode === 'issue' ? 'Medium' : 'Would be helpful'}
              </SelectItem>
              <SelectItem key="high">
                {mode === 'issue' ? 'High' : 'Important'}
              </SelectItem>
              <SelectItem key="critical">
                {mode === 'issue' ? 'Critical' : 'Essential'}
              </SelectItem>
            </Select>
            ...
            <div className="flex gap-4">
              ...
              <Button
                type="submit"
                isDisabled={loading || !title.trim() || !description.trim()}
                color="primary"
