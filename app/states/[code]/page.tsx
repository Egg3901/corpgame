...
export default function StateDetailPage() {
  ...
  const handleEnterMarket = async () => {
    ...
    try {
      ...
      alert(getErrorMessage(err, 'Failed to enter market'));
    } finally {
      setEntering(false);
    }
  };
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      {availableSectors.map((sector) => (
                        <SelectItem key={sector} textValue={sector}>
                          {sector}
                        </SelectItem>
                      ))}
                    </Select>
                    ...
                    {selectedSector && (
                      <div className="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-4">
                        ...
                        <div className="text-xs text-emerald-700 dark:text-emerald-300 space-y-2">
                          <p>Cost: {formatCurrency(MARKET_ENTRY_COST)} capital + {ACTIONS_CONFIG.COSTS.MARKET_ENTRY} actions</p>
                    ...
                    <Button
                      ...
                      isDisabled={!selectedSector || entering || user_corporation.capital < MARKET_ENTRY_COST || userActions < 1}
                      className="w-full font-semibold"
                      color="success"
                      isLoading={entering}
                      startContent={!entering && <Plus className="h-5 w-5" />}
                    >
                      {entering ? 'Entering Market...' : `Enter ${state.name} ${selectedSector || '[Select Sector]'} Market`}
                    </Button>
                  </div>
                </div>
              </div>
