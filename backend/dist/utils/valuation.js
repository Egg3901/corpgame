...
exports.calculateBusinessUnitAssets = calculateBusinessUnitAssets;
exports.calculateBalanceSheet = calculateBalanceSheet;
exports.calculateTradeWeightedPrice = calculateTradeWeightedPrice;
exports.calculateStockPrice = calculateStockPrice;
 ...
 */
async function calculateBusinessUnitAssets(corporationId) {
    const entries = await MarketEntry_1.MarketEntryModel.findByCorporationIdWithUnits(corporationId);
    let totalValue = 0;
    let retailValue = 0;
    ...
    for (const entry of entries) {
        ...
        const assetValues = (0, sectors_1.getMarketEntryAssetValue)(entry.sector_type, entry.state_code, entry.retail_count, entry.production_count, entry.service_count, entry.extraction_count);
 ...
 */
async function calculateBalanceSheet(corporationId) {
 ...
 */
async function calculateTradeWeightedPrice(corporationId) {
    const transactions = await ShareTransaction_1.ShareTransactionModel.getRecentActivity(corporationId, sectors_1.STOCK_VALUATION.TRADE_LOOKBACK_HOURS);
    ...
    sortedTransactions.forEach((tx, index) => {
        ...
        const recencyWeight = Math.pow(sectors_1.STOCK_VALUATION.RECENCY_DECAY, index);
 ...
 */
async function calculateStockPrice(corporationId) {
    const corporation = await Corporation_1.CorporationModel.findById(corporationId);
    if (!corporation) {
        throw new Error('Corporation not found');
    }
    ...
    const [balanceSheet, tradeData, finances] = await Promise.all([
        ...
        MarketEntry_1.MarketEntryModel.calculateCorporationFinances(corporationId, undefined, {
    ...
    calculatedPrice = Math.max(sectors_1.STOCK_VALUATION.MIN_SHARE_PRICE, calculatedPrice);
 ...
 */
async function updateStockPrice(corporationId, applyVariation = false) {
    ...
    if (applyVariation) {
        ...
        const variationPercent = sectors_1.STOCK_VALUATION.HOURLY_VARIATION_PERCENT;
        ...
        finalPrice = Math.max(sectors_1.STOCK_VALUATION.MIN_SHARE_PRICE, finalPrice);
        // Round to 2 decimal places
