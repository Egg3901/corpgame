...
const requestIp_1 = require("../utils/requestIp");
...
router.post('/register', async (req, res) => {
    try {
        const { email, username, password, player_name, gender, age, starting_state } = req.body;
        const registrationSecret = req.body?.registration_secret?.trim();
        const adminSecretAttempt = req.body?.admin_secret?.trim();
        const clientIp = (0, requestIp_1.getClientIp)(req);
        ...
        if (await BannedIp_1.BannedIpModel.isIpBanned(clientIp)) {
        ...
        const existingUser = await User_1.UserModel.findByEmail(trimmedEmail);
        if (existingUser) {
            return res.status(400).json({ error: 'User with this email already exists' });
        }
        // Check if username already exists (case-insensitive)
        const existingUsername = await connection_1.default.query('SELECT id FROM users WHERE LOWER(username) = LOWER($1)', [trimmedUsername]);
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, username: user.username }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '24h' });
...
router.post('/login', async (req, res) => {
    try {
        ...
        try {
            if (email) {
                user = await User_1.UserModel.findByEmail(identifier);
            }
            if (!user) {
                user = await User_1.UserModel.findByUsername(identifier);
            }
        }
        catch (dbError) {
        ...
        await User_1.UserModel.updateLastLogin(user.id, clientIp);
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, username: user.username }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '24h' });
        res.json({
            token,
            user: {
